...
import { Box, Typography, Stack, Fade, IconButton, useMediaQuery, useTheme } from '@mui/material';
...
export default function RestaurantMap({ restaurants = [] }) {
  ...
  // ─── Render ────────────────────────────────────────────────────────────────
  return (
    <Box sx={{ width: '100%', position: 'relative', display: 'flex', flexDirection: 'column', gap: 1, py: 2 }}>
      ...
      {selectedRestaurant && isMobile && (
        <Box
          ...
          sx={{ position: 'fixed', inset: 0, zIndex: 1299, bgcolor: 'rgba(0,0,0,0.35)' }}
        />
      )}

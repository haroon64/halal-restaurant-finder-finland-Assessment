...
export default function SearchInput({ setQuery, restaurants }) {
  ...
  const triggerSearch = (value) => {
    setQuery(value); // This updates the Map and Main List
    setLocalInput(value); // Sync the input text
      ...
      {showDropdown && localInput && (
        <div className="search-dropdown" style={{
          ...
          <div 
            ...
            style={{ padding: '12px', cursor: 'pointer', backgroundColor: '#f9f9f9', borderBottom: '1px solid #eee' }}
          >
            <strong>Search for:</strong> "{localInput}"
          </div>

          {/* Individual Restaurant Suggestions */}
          {suggestions.map((res) => (
            <div 
              key={res.id}
            ...
            >
              <div>
                ...
                <div style={{ fontSize: '12px', color: '#888' }}>{res.city}</div>
              </div>
              <span style={{ fontSize: '10px', color: '#2e7d32', fontWeight: 600 }}>{res.cuisine}</span>
            </div>
          ))}
